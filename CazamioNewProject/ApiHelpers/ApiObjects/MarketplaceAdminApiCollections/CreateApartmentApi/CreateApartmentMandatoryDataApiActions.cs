...
namespace CazamioNewProject.CreateApartmentMandatoryDataApi
{
    public partial class ApartmentCreation
    {
        ...
        {
            // Получаем BuildingId из базы данных
            var buildingIdResult = BuildingsDbRequests.Buildings.GetBuildingIdNineNineNineEightSaintJohnsonPlace();
            long buildingId = buildingIdResult.AddressId; // Предполагаем, что AddressId содержит BuildingId
            ...
            var payload = new RequestCreateApartmentMandatoryData
            {
                ...
                BuildingId = buildingId // Используем значение из базы данных
            };

            return payload;
        }

        // Переместили этот метод внутрь класса ApartmentCreation
